...
function nectar_image_with_animation($atts, $content = null) { 
	...
	if(preg_match('/^\d+$/',$image_url)){
		...
		if (function_exists('wp_get_attachment_image_srcset')) {
			$image_srcset = 'srcset="';
			$image_srcset .= wp_get_attachment_image_srcset($image_url, 'full');
			$image_srcset .= '"';
		}
		...
		($title_labels == 'true') ? $title_label_output = '<h2 class="uppercase recent-posts-title">'.$recent_posts_title_text.'<a href="'. $posts_page_link.'" class="button"> / '. $recent_posts_link_text .'</a></h2>' : $title_label_output = null;
			...
			<div class="row blog-recent columns-<?php echo $columns; ?>" data-style="<?php echo $style; ?>" data-color-scheme="<?php echo $color_scheme; ?>">
				...
				<div class="col <?php echo $col_num; ?>">
					...
					<?php $author_ids = get_field( 'author' ); ?>
					
					<div class="top-bar">
						<h4><?php the_date(); ?><span class="meta-category"> | <?php the_category(', '); ?> </span> <span class="meta-comment-count"> | <a href="<?php comments_link(); ?>"></h4>
					</div>
					...
					<div class="post-header">
						...
						<span class="meta-author">
							...
							<h5>
								...
								<?php// if( has_category( 'Video' ) || has_category( 'Podcasts' ) ) : ?>
									 <!-- Featuring -->
								<?php// else : ?>
								...
								<?php $rowCount = count( get_field('authors_name') ); //GET THE COUNT ?>
										<?php $i = 1; ?>
								<?php foreach( get_field('authors_name') as $post ): 
									setup_postdata( $post ); ?>
									<?php the_title(); ?>
								...
								<?php $recent_posts_query->reset_postdata(); 
									endforeach;
									?>
					...
					<?php 
						...
						if($style == 'default') {
						   ...
						   <?php }
								...
								if ( has_post_thumbnail() ) { echo '<a class="home-featured" href="' . get_permalink() . '">' . get_the_post_thumbnail($post->ID, 'portfolio-thumb', array('title' => '')) . '</a>'; }
							}
						...
						?>
							...
							<?php if( has_category( 'Podcasts' ) ) : ?>
								...
								<?php the_excerpt(); echo '<a data-hover-text-color-override="#fff" data-hover-color-override="false" data-color-override="#000000" class="nectar-button see-through-2 large" href="' . get_permalink() . '">Listen to Podcast</a>'; ?>
								
							<?php elseif( has_category( 'Video' ) ) : ?>
								
								<?php the_excerpt(); echo '<a data-hover-text-color-override="#fff" data-hover-color-override="false" data-color-override="#000000" class="nectar-button see-through-2 large" href="' . get_permalink() . '">Watch Video</a>'; ?>
							
							<?php else : ?>
							
							<?php the_excerpt(); echo '<a data-hover-text-color-override="#fff" data-hover-color-override="false" data-color-override="#000000" class="nectar-button see-through-2 large" href="' . get_permalink() . '">Read More</a>'; endif;
			...
			if($project_style == 'fullscreen_zoom_slider') {
				...
				if ( $the_query->have_posts() ) {
							while ( $the_query->have_posts() ) {
								...
								$recent_projects_content .= '<div class="project-info"><div class="container normal-container"><h1>'. get_the_title(). '</h1><a href="'.$the_project_link.'">View Project</a></div></div>';
